        ...
        <ul class="list-none w-full">
                    ...
                    <RouterLink class="w-full" :to="{ name: 'task-details', params: { id: task.id } }">
                        <div class="flex w-full items-center justify-between">
                            <div class="flex items-center">
                                <h3 class="text-lg font-bold" :class="{ 'line-through': task.status === 'completed' }">
                                    {{ task.title }}</h3>
                            </div>
            ...
            '')
                        ...
                        <div class="w-1/12 flex flex-col gap-2 justify-evenly items-center">
                            <RouterLink :to="{ name: 'edit-task', params: { id: task.id } }">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor"
                                    class="size-6 text-green-700 hover:cursor-pointer">
                                    <path stroke-linecap="round" stroke-linejoin="round"
